  ...
  const main = showTabs ? (
    <Tabs defaultValue="demo" className="w-full items-start gap-4">
      ...
      >
        <TabsTrigger
          value="demo"
          className="h-auto flex-none cursor-pointer rounded-none border-0 bg-transparent px-0 py-0 text-base font-medium text-muted-foreground shadow-none after:bottom-[-6px] hover:text-foreground data-active:bg-transparent data-active:font-medium data-active:text-foreground data-active:shadow-none dark:data-active:bg-transparent  duration-200 ease-out"
        >
          Demo
        </TabsTrigger>
        <TabsTrigger
    ...
    >
      {description ? (
        <figure className="flex w-full flex-col gap-3">
          {main}
          <figcaption className="text-muted-foreground mx-auto max-w-prose px-4 text-center text-sm leading-normal mt-2 sm:mt-2 text-pretty [&_p]:inline [&_p]:text-sm [&_p]:leading-normal [&_a]:inline-flex [&_a]:text-sm">
            {description}
          </figcaption>
        </figure>
      ) : (
