...
Pass `superHover` from `super-hover/svelte` to [`{@attach}`](https://svelte.dev/docs/svelte/@attach) on the list root. It is an [attachment factory](https://svelte.dev/docs/svelte/@attach#Attachment-factories), pass options to `superHover(...)`, and Svelte handles cleanup when the element is removed or options change.
