...
<ApiPropList aria-label="UseSuperHoverOptions" anchorPrefix="svelte-super-hover-action">
  <ApiProp name="enabled" type="boolean" defaultValue="true">
    When `false`, the Svelte action does not mount Super Hover on the root.
  </ApiProp>
  ...
  <ApiProp name="moveEventType" type="string or false" defaultValue={<em className="text-muted-foreground">auto</em>}>
    Custom move event name, or `false` to disable move events. If neither `onMove` nor `moveEventType` is passed, the Svelte action disables move events for you.
  </ApiProp>
  ...
  <ApiProp name="onMove" type="(event: SuperHoverMoveEvent) => void" defaultValue="off">
    Runs on move events while an element is active. The action only listens for move events when `onMove` is passed.
  </ApiProp>
</ApiPropList>
