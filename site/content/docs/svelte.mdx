...
Pass `superHover` from `super-hover/svelte` to [`{@attach}`](https://svelte.dev/docs/svelte/@attach) on the list root. Call it from an attach function and return `() => hv.destroy()` as cleanup.
...
<ul {@attach hover} class="space-y-1">
...
In Svelte, pass `onEnter`, `onLeave`, and `onMove` to `superHover` inside your attach function.
...
<script lang="ts">
    ...
    const hover = (node: HTMLElement) => {
      const hv = superHover(node, { onEnter: handleEnter });
      return () => hv.destroy();
    };
</script>
...
<script lang="ts">
    ...
    const hover = (node: HTMLElement) => {
      const hv = superHover(node, { enabled: !prefersReducedMotion.current });
      return () => hv.destroy();
    };
</script>
...
`UseSuperHoverOptions` for `superHover` from `super-hover/svelte`, passed as the second argument inside your attach function.

If options need to update reactively, define a new attach function when they change — for example with `$derived.by`.
