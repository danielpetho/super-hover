...
<Preview
  ...
  description="A dropdown-style menu where `superhoverenter` keeps the detail panel in sync while the list scrolls under the pointer."
/>
...
<ApiPropList aria-label="UseSuperHoverOptions" anchorPrefix="svelte-super-hover-action">
  <ApiProp name="enabled" type="boolean" defaultValue="true">
    When `false`, the Svelte action does not mount SuperHover on the root.
  </ApiProp>
