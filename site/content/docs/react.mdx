...
<Preview
  ...
  description="A scrollable two-dimensional grid using `sweptHitTest`. Each crossed cell flashes briefly, even when fast pointer movement or diagonal scrolling would normally skip intermediate cells."
...
So if every crossed item matters and you control the layout, a bespoke Motion-style implementation may be the fastest path. If you want a reusable hover-like behavior that works on arbitrary elements, Super Hover handles that for you, with the performance tradeoff that comes from measuring real DOM.
